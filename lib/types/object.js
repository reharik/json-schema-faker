...
// TODO provide types
var objectType = function objectType(value, path, resolve, traverseCallback, fillAllProperties) {
  var props = {};

  var fillProps = function (value, propsToFill) {
    propsToFill().forEach(function (key) {
  ...
  var getPropsToFill = function () {
    return fillAllProperties
      ? allProps
      : requiredAndRandomOpts()
  };

  var requiredAndRandomOpts = function () {
    ...
    var randomOptProps = random.shuffle(optProps).slice(0, length);
    return Array.prototype.concat.apply(optProps, reqProps);
  };
  ...
  fillProps(value, getPropsToFill);
